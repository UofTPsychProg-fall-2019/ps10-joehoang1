...
setwd("~/Documents/GitHub/ps10-joehoang1")
# load in the data
...
Q1 <- ggplot(ipip, aes(x=exer, y=BMI, color=gender)) +
      geom_violin(aes(color = gender), trim = FALSE, position = position_dodge(0.9) ) +
      geom_boxplot(aes(color = gender), width = 0.15, position = position_dodge(0.9)) +
      theme_classic()
Q1
...
Q3 <- ggplot(ipip, aes(x=gender, y=logMedInc, color=exer)) +  
      stat_summary(geom = "bar", fun.y = mean, position = "dodge", fill=aes(exer)) + 
      coord_cartesian(ylim = c(10, 12)) +
      stat_summary(geom = "errorbar", fun.data = mean_cl_boot, position ="dodge", size=.2, width=.9)+ 
      theme_light()
Q3
...
#ipip.g <- ipip.l %>%  ddply(c("trait", "gender", "BMI_cat"), summarise,
                            value=mean(value))
...
#Q5 <- ggplot(ipip.g, aes(x=BMI_cat, y=value, group=trait, colour=trait)) +
      geom_line() +
      geom_point() +
...
ipip.g <- ipip.l %>% group_by(trait, gender, BMI_cat) %>% summarise_each(mean)

Q5 <- ggplot(ipip.g1, aes(x=BMI_cat, y=value, group=trait, colour=trait)) +
  geom_line() +
  geom_point() +
  facet_wrap(vars(gender)) + 
  theme_minimal()
